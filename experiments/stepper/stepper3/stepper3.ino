...
// Removeved for the stepper testing
...
void callback(char *topic, byte *payload, unsigned int length)
{
    ...
    if (root["command"] == "setServoPosition")
    {
        ...
        // set the maximum speeds here globally
        if (targetPosition >= 90) {
            stepSpeed = map(targetPosition, 90, 180, 0, 1000);
            stepper1.setMaxSpeed(stepSpeed);
        }
        else  {
            stepSpeed = map(targetPosition, 0, 89, 1000, 0);
            stepper1.setMaxSpeed(stepSpeed);
        }

        // set the number of steps for position A
        int numStepsA = (transitionTime/1000*stepSpeed)
        stepperPositions[servoNum - 1][stateIndex] = numStepsA;
        // set the number of steps for position B
        int numStepsB = (transitionTime/1000*stepSpeed)
        stepperPositions[servoNum - 1][stateIndex] = numStepsB;
